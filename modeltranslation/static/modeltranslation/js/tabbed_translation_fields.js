...
function getGroupedTranslationFields(new_only) {
    ...
    var selector = '.modeltranslation';
    if(new_only){
        selector = '.modeltranslation:not(.modeltranslation_tabbed)';
    }

    var translation_fields = $(selector).filter('input[type=text]:visible, textarea:visible'),
...
function createTabs(new_only) {
    var grouped_translations = getGroupedTranslationFields(new_only);
    var tabs = [];
    $.each(grouped_translations, function (name, languages) {
        ...
        $.each(languages, function (lang, el) {
            ...
            $(el).addClass('modeltranslation_tabbed');
        });
...
function createMainSwitch(tabs) {
    var grouped_translations = getGroupedTranslationFields(false),
        unique_languages = [],
        select = $('<select>');
    $.each(grouped_translations, function (name, languages) {
        $.each(languages, function (lang, el) {
...
$(function(){
    if ($('body').hasClass('change-form')) {
        createMainSwitch(createTabs(false));
    }
});
